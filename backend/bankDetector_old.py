...
def extract_balances_from_pdf(pdf_bytes):
    ...
    try:
        ...
        opening_patterns = [
            r'Opening\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Opening\s+Bal\.?[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'B/?F[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Brought\s+Forward[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Balance\s+B/?F[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Previous\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Beginning\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Start(?:ing)?\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Initial\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
        ]
        ...
        closing_patterns = [
            r'Closing\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Closing\s+Bal\.?[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'C/?F[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Carried\s+Forward[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Balance\s+C/?F[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Final\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Ending\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'End\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
            r'Available\s+Balance[:\s]*(?:INR|₹|Rs\.?|USD|\$)?\s*([0-9,]+\.?\d{0,2})\s*(Cr|Dr)?',
        ]
        ...
        for pattern in closing_patterns:
            ...
            if match:
                balance_val = match.group(1).replace(',', '')
                cr_dr = match.group(2) if len(match.groups()) > 1 and match.group(2) else ''
                closing_balance = {'Balance': balance_val + cr_dr, 'Source': 'PDF'}
                print(f"[EXTRACT] Closing Balance found: {closing_balance}")
                break
...
def detect_bank_from_pdf(pdf_bytes):
    ...
    try:
        # Method 1: Extract text from top 25% of first page
        ...
        if reference_logos:
            ...
            # Check only first 2 logos for speed
...
def calculate_opening_from_first_transaction(df):
    ...
    if balance_col:
        try:
            balance = str(first_row[balance_col]).replace(',', '').strip()
            debit = str(first_row[debit_col]).replace(',', '').strip() if debit_col else '0'
            credit = str(first_row[credit_col]).replace(',', '').strip() if credit_col else '0'
            ...
            opening = balance_val - credit_val + debit_val
            if opening > 0:
...
def process_bordered_pdf(pdf_bytes, filename):
    ...
    if not opening_balance:
        opening_balance = calculate_opening_from_first_transaction(final_df)
    if not opening_balance and len(final_df) > 0:
        for col in final_df.columns:
            if 'balance' in str(col).lower():
                first_balance = str(final_df.iloc[0][col]).strip()
                if first_balance and first_balance not in ['', '-', 'nan']:
                    opening_balance = {'Balance': first_balance, 'Source': 'FirstTransaction'}
    ...
    if not closing_balance and len(final_df) > 0:
        for col in final_df.columns:
            if 'balance' in str(col).lower():
                last_balance = str(final_df.iloc[-1][col]).strip()
                if last_balance and last_balance not in ['', '-', 'nan']:
                    closing_balance = {'Balance': last_balance, 'Source': 'LastTransaction'}
                    print(f"[FALLBACK] Closing Balance: {closing_balance}")
                break
